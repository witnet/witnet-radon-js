...
describe('Operator methods', () => {
  ...
  describe('getMarkup', () => {
    ...
    it.only('with subscript argument', () => {
      const cache = new Cache()
      const operator = new Operator(cache, 0, OutputType.SubscriptOutput, [OperatorCode.ArrayMap, [DEFAULT_OPERATOR]] , { emit: () => {} })
      const result = operator.getMarkup()
      const expected = {
          "hierarchicalType": "operator",
          "id": 1,
  ...
  describe('update', () => {
    describe('from operator code ', () => {
      ...
      it.only('map with subscript argument', () => {
        const cache = new Cache()
